...
一応普段の開発をやるのに支障はない程度の理解はあるが、インフラ変更のレビューでイマイチ良いのかわからない・トラブル時に見るべき箇所のイメージがわかない方
=>ECSのリソースや関係をざっくり理解して、上記のようなことに少し自信が持てるようにしよう
普段の運用程度では触れないような、初期構築や大きな変更に必要となるようなところには触れません（タスク実行ロールとタスクロールの違い、コンテナイメージ取得の通信経路、各種ヘルスチェックの詳細など）


* ECSとFargateは特段いいわけていません

下準備：AWSの周辺リソース一式を用意する
-----
本記事ではFargateによるタスクやサービスをじっくり作っていきますが、あくまで運用者を対象としていることもあり、Fargateコンテナを動かすために必要となる周辺リソースの話はほとんどしません。

しかしながら必要な周辺リソースは地味に多いため、ここでは拙作の周辺リソース一式構築セットを使います。

https://github.com/cumet04/fargate-skeleton/

<!-- TODO: タグ打つ -->

READMEの使い方セクションにある3要素のうち「AWSインフラ」のみを使います（appコンテナとecspressoの成分は記事内で作ります）。READMEに従いAWSインフラ部分を`terraform apply`すれば、3分クッキングよろしくリソースセットが出来上がります。
<!-- TODO: ALB使うかどうか、この時点で不要なら料金無い旨 -->
...
<!-- logConfigurationは略せるが、無いとマネコンでログ確認できないので出すしかない -->
<!-- awslogs-stream-prefixはFargateの場合は略せない。謎。 -->
<!-- TODO: taskexecroleについて何らか補足を入れたい -->
...
（まだ）カスタムイメージのビルドをしたくないので、少々強引にwebrickをインストールして[httpサーバを起動](https://docs.ruby-lang.org/ja/latest/library/un.html)しています。その他オプションとして、ルートディレクトリが見えると精神衛生上よろしくないので`/home`を指定し、後続のステップの都合上でポート3000を指定しています。
...
```
※例によって適用時はコメントを除去してください

<!-- "launchType": "FARGATE" は略せるが、略すとタスク起動できない。またupdate時は指定できない。なのでjsonよりcliで指定したほうがこの場合楽 -->
<!-- clusterはcliだと名前だがjsonだとarnになってめんどい。なのでcliで -->
<!-- taskDefinitionはarnでもfamiliy:revisionでもどっちでも良い -->
<!-- "assignPublicIp": "ENABLED" は略せるが、略すとイメージpullができない -->
